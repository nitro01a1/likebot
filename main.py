...
# ساخت اپلیکیشن تلگرام - این بخش به بیرون از تابع منتقل شد
application = (
    Application.builder()
    ...
    if context.args:
        try:
            ...
            if referrer_id != user.id:
                ...
                existing_user = database.get_or_create_user(referrer_id, "") # ensure referrer exists
                # اگر کاربر واقعا جدید بود و قبلا معرف نداشت، به معرف امتیاز بده
                if db_user and db_user.get('referred_by') == referrer_id:
                     # Check if we already credited for this referral
                    is_new_referral = not hasattr(context.bot_data, f'referred_{user.id}')
                    if is_new_referral:
                        database.update_points(referrer_id, 1)
                        context.bot_data[f'referred_{user.id}'] = True # Mark as credited
                        await context.bot.send_message(
                            chat_id=referrer_id,
                            text=f"کاربر جدیدی با نام «{user.first_name}» با لینک شما وارد ربات شد و ۱ امتیاز دریافت کردید. ✅\nمجموع امتیاز شما: {existing_user['points'] + 1}"
                        )
...
async def start_service_conversation(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    service_map = {
        'free_like': {'cost': 1, 'state': AWAITING_ID_LIKE, 'name': 'لایک رایگان🔥'},
        'account_info': {'cost': 1, 'state': AWAITING_ID_INFO, 'name': 'اطلاعات اکانت📄'},
        'free_stars': {'cost': 3, 'state': AWAITING_ID_STARS, 'name': 'استارز رایگان⭐'}
    }
    cost = service_map[service]['cost']
...
# Other admin handlers (ban, unban, add_points, etc.) are the same as before...
...
# این تابع در زمان استقرار وب‌هوک را تنظیم می‌کند
async def setup():
...
# در زمان شروع برنامه، یکبار وب‌هوک را تنظیم می‌کنیم
# این کار را در یک context جداگانه انجام می‌دهیم تا با فلسک تداخل نکند
