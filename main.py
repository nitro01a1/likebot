...
async def service_entry_point(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if service_key == 'free_stars':
        await update.message.reply_text(f"✅ {cost} امتیاز کسر شد. موجودی جدید: {new_points}.\n\nلطفا ایدی عددی حساب خود در ربات، لینک کانال و پست را در یک متن ارسال کنید.\nبرای لغو /cancel را بزنید.")
        return AWAITING_STARS_DETAILS
    else:
        await update.message.reply_text(f"✅ {cost} امتیاز کسر شد. موجودی جدید: {new_points}.\n\nبرای تکمیل سفارش، آیدی عددی خود را ارسال کنید.\nبرای لغو /cancel را بزنید.")
        return AWAITING_ID

async def receive_id_and_process(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    user = update.effective_user; game_id = update.message.text; service_key = context.user_data.get('service_key')
    if not game_id.isdigit():
        await update.message.reply_text("❌ ورودی نامعتبر است. لطفاً فقط عدد وارد کنید."); return AWAITING_ID
    if not (5 <= len(game_id) <= 14):
        await update.message.reply_text("❌ تعداد ارقام باید بین ۵ تا ۱۴ باشد. لطفاً مجدداً تلاش کنید."); return AWAITING_ID
    ...
    sent_message = await update.message.reply_text(f"درخواست شما با موفقیت ثبت شد.\nامتیاز شما: {database.get_or_create_user(user.id, user.first_name)['points']}")
