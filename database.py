...
def init_db():
    """
    پایگاه داده و جداول پایه را ایجاد می‌کند.
    (بدون قابلیت‌های VIP و قرعه‌کشی)
    """
    with sqlite3.connect(DB_NAME) as conn:
        cursor = conn.cursor()

        # جدول کاربران (نسخه ساده)
        cursor.execute('''
        ...
        # جدول پیگیری استفاده کاربران از کدهای هدیه
        cursor.execute('''
...
def get_all_user_ids(only_active=True):
    """فقط آیدی تمام کاربران را برای ارسال پیام همگانی برمی‌گرداند."""
    with sqlite3.connect(DB_NAME) as conn:
        cursor = conn.cursor()
